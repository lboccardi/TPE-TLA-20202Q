...
var
    ...
    | CHAR ALPHA ASSIGN ALPHA ARRAY ALPHA           { if(!isOfKind($6,KIND_INT)){yyerror("Variable should be int type to de-reference.");YYABORT;}if(!correctArray($4,KIND_ARRAY_CHAR,atoi($6))){yyerror("Variables aren't compatible.");YYABORT;}$$ = malloc(4+strlen($2)+6+strlen($4)+strlen($6)); sprintf($$, "char %s=%s[%s]", $2, $4, $6);add($$,true);addVar($2, KIND_CHAR,1);}
    ...
    | ALPHA ARRAY ALPHA ASSIGN ALPHA            { if (!isOfKind($1, KIND_ARRAY_CHAR)){yyerror("Variable should be char array type"); YYABORT;} if(! enoughSpace($1, atoi($3)) ) { yyerror("Array has not enough space"); YYABORT; } if(!isOfKind($3,KIND_INT)){yyerror("Variable should be int type."); YYABORT;}if(!isOfKind($5,KIND_CHAR)){yyerror("Variable should be char type."); YYABORT;} $$ = malloc(strlen($1) + strlen($3) + strlen($5) + 4 ); sprintf($$, "%s[%s]=%s", $1, $3, $5); add($$, true); }
    ; 
