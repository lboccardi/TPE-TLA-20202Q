...
var
    ...
    | ALPHA ARRAY ALPHA ASSIGN ALPHA            { if(!compatibleArray($5,$1,0)){yyerror("Variables aren't compatible.");YYABORT;} if (!isOfKind($3, KIND_INT)){yyerror("Variable should be int type"); YYABORT;} if(!isOfKind($3,KIND_INT)){yyerror("Variable should be int type."); YYABORT;}$$ = malloc(strlen($1) + strlen($3) + strlen($5) + 4 ); sprintf($$, "%s[%s]=%s", $1, $3, $5); add($$, true); }
    ; 
