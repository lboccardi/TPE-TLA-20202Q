...
var
    ...
    | ALPHA ARRAY ALPHA ASSIGN ALPHA ARRAY ALPHA{ if(!checkIfVarExists($5) || !checkIfVarExist($1)){yyerror("That array variable does not exist"); YYABORT;}  if(!compatibleArrayAssignment($1,$5,atoi($3),atoi($7))){yyerror("Arrays types aren't compatible.");YYABORT;} if(!isOfKind($3,KIND_INT)){yyerror("First index should be int type."); YYABORT;} if(!isOfKind($7,KIND_INT)){yyerror("Second index should be int type."); YYABORT;} $$ = malloc(strlen($1) + strlen($3) + strlen($5) +strlen($7) + 7 ); sprintf($$, "%s[%s]=%s[%s]", $1, $3, $5,$7); add($$, true); }  
