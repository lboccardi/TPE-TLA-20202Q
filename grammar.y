...
var
    ...
    | STRING ALPHA ASSIGN ALPHA                     { printf("char alpha assign escape alpha");if(checkIfVarExists($2)){yyerror("That variable already exists, please choose another name"); YYABORT;}if(!isOfKind($4,KIND_STRING)){yyerror("Variable should be string type"); YYABORT;}$$ = malloc(7+strlen($2)+6+strlen($4)); sprintf($$,"%s %s=%s","char *",$2,$4); add($$,true); addVar($2, KIND_STRING,1);}
    | CHAR ALPHA ASSIGN ESCAPE ALPHA                { if(strcmp($2, "char")==0){yyerror("That name is not permmited. Please choose another one"); YYABORT;};if(checkIfVarExists($2)){yyerror("That variable already exists, please choose another name"); YYABORT;}if(!isOfKind($4,KIND_CHAR)){yyerror("Variable should be char type."); YYABORT;} if(strlen($5)!=1){yyerror("Incompatible initialization for type char."); YYABORT;}if(checkIfVarExists($2)){yyerror("That variable already exists, please choose another name"); YYABORT;}$$ = malloc(7+strlen($2)+6+strlen($5));sprintf($$,"char %s=\'%s\'",$2,$5); add($$,true); addVar($2, KIND_CHAR,1);} 
    | CHAR ALPHA ASSIGN ESCAPE DIGIT                { if(strcmp($2, "char")==0){yyerror("That name is not permmited. Please choose another one"); YYABORT;};if(checkIfVarExists($2)){yyerror("That variable already exists, please choose another name"); YYABORT;}if(!isOfKind($4,KIND_CHAR)){yyerror("Variable should be char type."); YYABORT;} if(strlen($5)!=1){yyerror("Incompatible initialization for type char."); YYABORT;}if(checkIfVarExists($2)){yyerror("That variable already exists, please choose another name"); YYABORT;}$$ = malloc(7+strlen($2)+6+strlen($5));sprintf($$,"char %s=\'%s\'",$2,$5); add($$,true); addVar($2, KIND_CHAR,1);} 
    ...
    | INT ALPHA ASSIGN CONVERT OPEN_P ALPHA CLOSE_P { if(!isOfKind($6,KIND_STRING) && !isOfKind($6,KIND_ARRAY_CHAR)){yyerror("Variable to convert should be string type."); YYABORT;} $$ = malloc(7+strlen($6)+3+strlen($2)+6); sprintf($$,"int %s = atoi(%s)",$2,$6); add($$,true); addVar($2, KIND_INT,1);}
    | INT ALPHA ASSIGN LENGTH OPEN_P ALPHA CLOSE_P  { if(!isOfKind($6,KIND_STRING) && !isOfKind($6,KIND_ARRAY_CHAR)){yyerror("Variable should be string type."); YYABORT;}$$ = malloc(strlen($6)+strlen($2) +15); sprintf($$,"int %s=strlen(%s)",$2, $6); add($$,true);addVar($2, KIND_INT,1);}      
    ...
    | INT ALPHA ASSIGN ALPHA ARRAY ALPHA            { if(!isOfKind($6,KIND_INT)){yyerror("Variable should be int type to de-reference.");YYABORT;}if(!isOfKind($4,KIND_ARRAY_INT)){yyerror("Variables aren't compatible.");YYABORT;}$$ = malloc(3+strlen($2)+6+strlen($4)+strlen($6)); sprintf($$, "int %s=%s[%s]", $2, $4, $6);add($$,true);addVar($2, KIND_INT,1);}
    ...
    | CHAR ALPHA ASSIGN ALPHA                       { if(strcmp($2, "char")==0){yyerror("That name is not permmited. Please choose another one"); YYABORT;};if(checkIfVarExists($2)){yyerror("That variable already exists, please choose another name"); YYABORT;}if(!isOfKind($4,KIND_CHAR)){yyerror("Variable should be char type."); YYABORT;} if(strcmp($2, "char")==0){yyerror("That name is not permitted. Please choose another one"); YYABORT;};if(checkIfVarExists($2)){yyerror("That variable already exists, please choose another name"); YYABORT;}if(!isOfKind($4,KIND_CHAR)){yyerror("Variable should be char type."); YYABORT;}$$ = malloc(4+strlen($2)+6+strlen($4)); sprintf($$,"%s %s=%s","char",$2,$4); add($$,true); addVar($2, KIND_CHAR,1);}
    | CHAR ALPHA ASSIGN ALPHA ARRAY DIGIT           { if(strcmp($2, "char")==0){yyerror("That name is not permmited. Please choose another one"); YYABORT;};if(checkIfVarExists($2)){yyerror("That variable already exists, please choose another name"); YYABORT;}if(!isOfKind($4,KIND_CHAR)){yyerror("Variable should be char type."); YYABORT;}  if(!correctArray($4,KIND_ARRAY_CHAR,atoi($6))){yyerror("Variables aren't compatible.");YYABORT;}$$ = malloc(4+strlen($2)+6+strlen($4)+strlen($6)); sprintf($$, "char %s=%s[%s]", $2, $4, $6);add($$,true);addVar($2, KIND_CHAR,1);}
    | CHAR ALPHA ASSIGN ALPHA ARRAY ALPHA           { if(strcmp($2, "char")==0){yyerror("That name is not permmited. Please choose another one"); YYABORT;};if(checkIfVarExists($2)){yyerror("That variable already exists, please choose another name"); YYABORT;}if(!isOfKind($4,KIND_CHAR)){yyerror("Variable should be char type."); YYABORT;} if(!isOfKind($6,KIND_INT)){yyerror("Variable should be int type to de-reference.");YYABORT;}if(!isOfKind($4,KIND_ARRAY_CHAR)){yyerror("Variables aren't compatible.");YYABORT;}$$ = malloc(4+strlen($2)+6+strlen($4)+strlen($6)); sprintf($$, "char %s=%s[%s]", $2, $4, $6);add($$,true);addVar($2, KIND_CHAR,1);}
...
char_array
    : ESCAPE ALPHA COMMA char_array { printf("en escape alpha comma"); if(strlen($2)!=1){yyerror("Incompatible initialization for type char."); YYABORT;}$$ = malloc(strlen($2)+5+strlen($4)); sprintf($$, "\'%s\',%s", $2,$4); add($$,true);}
    | ESCAPE ALPHA                  { printf("en escape alpha"); if(strlen($2)!=1){yyerror("Incompatible initialization for type char."); YYABORT;}$$ = malloc(strlen($2)+5); sprintf($$, "\'%s\'", $2); add($$,true);}
